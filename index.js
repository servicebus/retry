...
module.exports = function (options = {}) {
  ...
  function createOnly (method, max) {
    ...
    return function only (message) {
      ...
      if (
        Object.keys(calledByMethod).length &&
        calledByMethod[method] > methodMax[method]
      ) {
  ...
  async function setRetriesRemaining (uniqueMessageId, message, cb) {
    ...
    if (!options.setRetriesRemaining) {
      ...
      // rabbitmq
      return store.get(getNamespacedUniqueMessageId(uniqueMessageId), function (
        err,
        count
      ) {
      ...
      // log('kafka detected!')
      const namespacedUniqueMessageId = getNamespacedUniqueMessageId(
        uniqueMessageId
      )
      ...
      setRetriesRemaining(uniqueMessageId, msg, function (err, message) {
        ...
        message.content.handle = {
          ack: async function ack (cb) {
            ...
            if (channel.ack) {
              ...
              return await store.ack(
                getNamespacedUniqueMessageId(uniqueMessageId),
                cb
              )
            }
          ...
          reject: function reject (cb) {
            ...
            var namespacedUniqueMessageId = getNamespacedUniqueMessageId(
              uniqueMessageId
            )
            ...
            store.increment(namespacedUniqueMessageId, function (err) {
              ...
              store.get(namespacedUniqueMessageId, function (err, count) {
                ...
                if (count > maxRetries) {
                  var errorQueueName = util.format(
                    '%s.error',
                    options.queueName
                  )
                  ...
                  channel.sendToQueue(
                    errorQueueName,
                    buffer,
                    extend(options, { headers: { rejected: count } })
                  )
