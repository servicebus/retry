...
describe('retry', function () {
  describe('MemoryStore', function () {
    ...
    it('should throw if ack called more than once on message', function () {
      ...
      middleware(channel, message, { ack: true }, function (
        err,
        channel,
        message,
        options,
    ...
    it('should throw if reject called more than max on message', function () {
      ...
      var middleware = retry().handleIncoming
      middleware(channel, message, { ack: true, maxRetries: 3 }, function (
        err,
        channel,
        message,
        options,
    ...
    it('rejected send/listen messages should retry until max retries', function (done) {
      ...
      bus.listen('test.servicebus.retry.1.error', { ack: true }, function (
        event
      ) {
        count.should.equal(4) // one send and three retries
        event.handle.ack()
        bus
          .destroyListener('test.servicebus.retry.1')
          .on('success', function () {
            bus
              .destroyListener('test.servicebus.retry.1.error')
              .on('success', function () {
                done()
              })
          })
    ...
    it('rejected publish/subscribe messages should retry until max retries', function (done) {
      ...
      bus.listen('test.servicebus.retry.2.error', { ack: true }, function (
        event
      ) {
        count.should.equal(4) // one send and three retries
        event.handle.ack()
        ...
        bus
          .destroyListener('test.servicebus.retry.2.error')
          .on('success', function () {
            done()
          })
        // });
  ...
  describe('RedisStore', function () {
    ...
    it('rejected send/listen messages should retry until max retries', function (done) {
      ...
      bus.listen('test.servicebus.retry.3.error', { ack: true }, function (
        event
      ) {
        count.should.equal(4) // one send and three retries
        event.handle.ack()
        bus
          .destroyListener('test.servicebus.retry.3')
          .on('success', function () {
            bus
              .destroyListener('test.servicebus.retry.3.error')
              .on('success', function () {
                done()
              })
          })
    ...
    it('rejected publish/subscribe messages should retry until max retries', function (done) {
      ...
      bus.listen('test.servicebus.retry.4.error', { ack: true }, function (
        event
      ) {
        count.should.equal(4) // one send and three retries
        event.handle.ack()
        // subscription.unsubscribe(function () {
        bus
          .destroyListener('test.servicebus.retry.4.error')
          .on('success', function () {
            done()
          })
        // });
    ...
    it('rejected wildcard subscribe messages should retry until max retries', function (done) {
      var count = 0
      var subscription = bus.subscribe(
        'test.servicebus.retry.wildcard.*',
        { ack: true },
        function (event) {
          count++
          event.handle.reject()
      ...
      bus.listen(
        'test.servicebus.retry.wildcard.*.error',
        { ack: true },
        function (event) {
          count.should.equal(4) // one send and three retries
          event.handle.ack()
          // subscription.unsubscribe(function () {
          bus
            .destroyListener('test.servicebus.retry.wildcard.*.error')
            .on('success', function () {
              done()
            })
          // });
  ...
  describe('options', function () {
    describe('namespace', function () {
      ...
      it('should prepend unique message id with provided namespace', function (done) {
        ...
        bus.listen('test.servicebus.retry.5.error', { ack: true }, function (
          event
        ) {})
          ...
          if (count === 4) {
            ...
            store.get(key, function (err, rejectCount) {
              ...
              store.clear(key, function (err) {
                if (err) return done(err)
                bus
                  .destroyListener('test.servicebus.retry.5', { force: true })
                  .on('success', function () {
                    bus
                      .destroyListener('test.servicebus.retry.5.error', {
                        force: true
                      })
                      .on('success', function () {
                        done()
    ...
    describe('setRetriesRemaining', function () {
      it('should provide count of retries remaining', function (done) {
        ...
        bus.listen('test.servicebus.retry.6', { ack: true }, function (event) {
          ...
          if (event.retriesRemaining === 0) {
            bus
              ...
              .on('success', function () {
                bus
                  .destroyListener('test.servicebus.retry.6.error', {
                    force: true
                  })
                  .on('success', function () {
                    done()
        ...
        bus.listen('test.servicebus.retry.6.error', { ack: true }, function (
          event
        ) {})
